...
where
    T: Read + Seek,
{
    read_header_with_salt(fname, rd, None)
}

pub fn read_header_with_salt<T>(fname: &str, rd: &mut T, salt: Option<&str>) -> Result<FileHeader, Error>
where
    T: Read + Seek,
{
    let key = encryption::gen_header_key_with_salt(&fname, salt);
...
where
    T: Read + Seek,
{
    read_entries_with_salt(fname, header, rd, None)
}

pub fn read_entries_with_salt<T>(
    fname: &str,
    header: &FileHeader,
    rd: &mut T,
    salt: Option<&str>,
...
where
    T: Read + Seek,
{
    let key = encryption::gen_entries_key_with_salt(&fname, salt);
