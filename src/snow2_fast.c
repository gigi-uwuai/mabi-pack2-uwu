...
void c_snow2_loadkey(unsigned int *state_table, const char *key)
{
    Snow2_ctx ctx;
    ctx.keysize = 128;
    memcpy(ctx.key, key, 16);
    snow_loadkey_fast(&ctx, 0, 0, 0, 0);

    state_table[0]=ctx.s15;
    state_table[1]=ctx.s14;
    state_table[2]=ctx.s13;
    state_table[3]=ctx.s12;
    state_table[4]=ctx.s11;
    state_table[5]=ctx.s10;
    state_table[6]=ctx.s9;
    state_table[7]=ctx.s8;
    state_table[8]=ctx.s7;
    state_table[9]=ctx.s6;
    state_table[10]=ctx.s5;
    state_table[11]=ctx.s4;
    state_table[12]=ctx.s3;
    state_table[13]=ctx.s2;
    state_table[14]=ctx.s1;
    state_table[15]=ctx.s0;
    state_table[16]=ctx.r1;
    state_table[17]=ctx.r2;
}

void c_snow2_generate_keystream(int *state_table, int *out_stream)
{
    Snow2_ctx ctx;
    ctx.s15=state_table[0];
    ctx.s14=state_table[1];
    ctx.s13=state_table[2];
    ctx.s12=state_table[3];
    ctx.s11=state_table[4];
    ctx.s10=state_table[5];
    ctx.s9=state_table[6];
    ctx.s8=state_table[7];
    ctx.s7=state_table[8];
    ctx.s6=state_table[9];
    ctx.s5=state_table[10];
    ctx.s4=state_table[11];
    ctx.s3=state_table[12];
    ctx.s2=state_table[13];
    ctx.s1=state_table[14];
    ctx.s0=state_table[15];
    ctx.r1=state_table[16];
    ctx.r2=state_table[17];

    snow_keystream_fast(&ctx, (u32*)out_stream);
