...
pub fn gen_header_key_with_salt(name: &str, salt: Option<&str>) -> Vec<u8> {
    let salt_to_use = salt.expect("Missing encryption salt! Ensure 'salt_keys.json' exists.");
    let input: Vec<u16> = (name.to_ascii_lowercase() + salt_to_use)
        .encode_utf16()
        .collect();
    (0..128)
...
pub fn gen_entries_key_with_salt(name: &str, salt: Option<&str>) -> Vec<u8> {
    let salt_to_use = salt.expect("Missing encryption salt! Ensure 'salt_keys.json' exists.");
    let input: Vec<u16> = (name.to_ascii_lowercase() + salt_to_use)
        .encode_utf16()
        .collect();
...
mod tests {
    ...
    #[test]
    fn header_key() {
        let key = gen_header_key_with_salt("data_00000.it", Some("})wWb4?-sVGHNoPKpc"));
        assert_eq!(
            key[..16],
            [
    ...
    #[test]
    fn entries_key() {
        let key = gen_entries_key_with_salt("data_00000.it", Some("})wWb4?-sVGHNoPKpc"));
        assert_eq!(
            key[..16],
            [
