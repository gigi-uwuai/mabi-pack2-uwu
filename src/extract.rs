...
pub fn run_extract(fname: &str, output_folder: &str, filters: Vec<&str>) -> Result<(), Error> {
    // Preserve existing API for CLI callers: ignore the count of extracted files.
    run_extract_with_salt(fname, output_folder, filters, None).map(|_| ())
}

/// Extract with an optional salt, returning how many entries were actually extracted.
/// The count respects the provided filters (0 means no entries matched).
pub fn run_extract_with_salt(
    fname: &str,
    output_folder: &str,
    filters: Vec<&str>,
    salt: Option<&str>,
